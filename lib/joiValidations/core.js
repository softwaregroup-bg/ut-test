...
module.exports = {
    ...
    validateListExternalSystemType: function(result) {
        var schema = joi.array().items(joi.object().keys({
            ...
            name: joi.string().required(),
            depthKey: joi.string().allow(null).required(),
            value: joi.number().integer().allow(null).required(),
            isBranchUnique: joi.boolean().allow(0, 1, '0', '1').required(),
            isSingleUser: joi.boolean().allow(0, 1, '0', '1').required()
        })
        ).required();
        return joi.validate(result, schema, {abortEarly: false});
    },
    ...
    validateServerInfo: function(result) {
        var schema = joi.array().items(joi.object().keys({
            ...
            serverTimeout: joi.number().allow(null),
            encryptionType: joi.string().allow(null)
        })
        ).required();
        return joi.validate(result, schema, {abortEarly: false});
    },
    ...
    /**
     * @param {object} result - result to validate (result.resultSet)
     * @return {joiObject} - joiValidation
     */
    validateListItemNameByParent: function(result) {
        var schema = joi.array().items(joi.object().keys({
