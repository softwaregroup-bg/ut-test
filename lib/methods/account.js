...
module.exports = {
    ...
    editAccount: function(stepname, contextStep) {
        return commonFunc.createStep('ledger.account.edit', stepname, context => accountParams.editAccountParams(context, contextStep),
        (result, assert) => {
            assert.equals(accountJoiValidation.validateEditAccount(result.account).error, null, 'return all detais after editting an account');
        });
    }
};
