...
module.exports = {
    ...
    fetchAccountBalance: function(stepname, contextStep, expectedBalance) {
        return commonFunc.createStep('ledger.account.fetch', stepname, context => {
            return {
                filterBy: {
                    accountNumber: contextStep(context)
                }
            };
        },
        (result, assert) => {
