...
module.exports = {
    generateAdmin: function() {
        return commonFunc.createStep('userTest.user.generate', 'generateAdmin', context => {
            ...
            assert.comment('actorId: ' + result.actor.actorId);
    ...
    loginAdmin: function(stepname, contextStep) {
        return {
            ...
            name: stepname,
            params: context => userParams.loginAdminParams(context, contextStep),
            $meta: {
                ...webMeta
            },
            result(result, assert) {
                const {error} = userJoiValidation.validateLogin(result['identity.check']);
                assert.equals(error, null, 'Return all details after login a user');
                if (this.loginMeta) {
                    this.$meta = {
                        auth: {
                            timezone: '+00:00',
    ...
    loginMobile: function(stepname, username, password, timezone, newPassword) {
        return {
            ...
            result(result, assert) {
                const {error} = userJoiValidation.validateLogin(result['identity.check']);
                assert.equals(error, null, 'Return all details after login a user - mobile');
                if (this.loginMeta) {
                    this.$meta = {
                        auth: {
                            timezone: '+00:00',
    ...
    loginError: function(stepname, username, password, timezone) {
        return {
            method: 'identity.check',
            name: stepname,
            params: {
    ...
    addRole: function(stepname, contextStep, name, description) {
        return commonFunc.createStep('user.role.add', stepname, context => userParams.addRoleParams(context, contextStep, name, description),
            (result, assert) => {
                assert.comment('actorId: ' + result.role[0].actorId);
    ...
    addExternalUser: function(stepname, contextStep, username, externalSystemId) {
        return commonFunc.createStep('user.externalUser.add', stepname, context => userParams.addExternalUserParams(context, contextStep, username, externalSystemId),
            (result, assert) => {
                assert.comment('actorId: ' + result.addedExternalUsers[0].externalUserId);
