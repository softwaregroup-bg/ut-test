...
module.exports = {
    ...
    fetchActionLog: function(stepname, contextStep) {
        return commonFunc.createStep('user.action.fetch', stepname, context => userParams.fetchActionLogParams(context, contextStep),
        (result, assert) => {
            assert.equals(userJoiValidation.validateFetchActions(result.actions).error, null, 'return user actions');
        });
    },
    fetchAuditLog: function(stepname, contextStep) {
        return commonFunc.createStep('audit.audit.fetch', stepname, context => userParams.fetchAuditLogParams(context, contextStep),
        (result, assert) => {
            assert.equals(userJoiValidation.validateFetchActionLog(result.actionLog).error, null, 'return all details after fetching audit log');
        });
    },
    editAction: function(stepname, contextStep) {
        return commonFunc.createStep('user.action.edit', stepname, context => userParams.editActionParams(context, contextStep),
        (result, assert) => {
            assert.equals(userJoiValidation.validateEditAction(result.action).error, null, 'return all details after editing action');
        });
    },
    getAuditLog: function(stepname, contextStep) {
        return commonFunc.createStep('audit.audit.get', stepname, context => userParams.getAuditLogParams(context, contextStep),
        (result, assert) => {
            assert.equals(userJoiValidation.validateGetAuditLog(result.auditLog).error, null, 'Return all details after getting audit log');
        });
    }
};
