...
module.exports = {
    ...
    fetchActionLog: function(stepname, contextStep) {
        return commonFunc.createStep('user.actionLog.fetch', stepname, context => userParams.fetchActionLogParams(context, contextStep),
        (result, assert) => {
            console.log(result);
            assert.equals(userJoiValidation.validateFetchActionLog(result.actionLog).error, null, 'return all details after fetching action log');
        });
    }
};
