...
const customerParams = require('./../requestParams/customer');
...
module.exports = {
    getCustomer: function(stepname, contextStep, firstName) {
        return commonFunc.createStep('customer.customer.get', stepname, context => customerParams.actorIdParams(context, contextStep),
            (result, assert) => {
    ...
    addCustomer: function(stepname, contextStep, firstName, stateId, statusId) {
        return commonFunc.createStep('customer.customer.add', stepname, context => customerParams.addCustomerParams(context, contextStep, firstName, stateId, statusId),
            (result, assert) => {
                assert.comment('actorId: ' + result.person.actorId);
    ...
    addKyc: function(stepname, contextStep, kycDescription) {
        return commonFunc.createStep('customer.kyc.add', stepname, context => customerParams.addKycParams(context, contextStep, kycDescription),
            (result, assert) => {
                assert.comment('kycId: ' + result.kyc[0].kycId);
    ...
    editKyc: function(stepname, contextStep, kycDescription) {
        return commonFunc.createStep('customer.kyc.edit', stepname, context => customerParams.editKycParams(context, contextStep, kycDescription),
            (result, assert) => {
    ...
    getByDepthOrganization: function(stepname, contextStep) {
    ...
    fetchCountry: function(stepname, contextStep) {
        return commonFunc.createStep('core.country.fetch', 'fetch countries', (context) => {
            return {};
        }, (result, assert) => {
    ...
    addReferral: function(stepname, contextStep) {
        return commonFunc.createStep('customer.referral.add', stepname, context => customerParams.addReferralParams(context, contextStep),
            (result, assert) => {
                assert.comment('referralId: ' + result.referral[0].referralId);
                ...
                assert.true((result.referral[0].referralStatus).toString().toLowerCase().indexOf('New'.toLowerCase()) > -1, 'return new referral status');
                assert.true(result.referral[0].isNew, 'return isNew = true');
            });
    },
    getReferral: function(stepname, contextStep) {
        return commonFunc.createStep('customer.referral.get', stepname, context => customerParams.referralIdParams(context, contextStep),
            (result, assert) => {
    ...
    deleteReferral: function(stepname, contextStep) {
        return commonFunc.createStep('customer.referral.delete', stepname, context => customerParams.referralIdParams(context, contextStep),
            (result, assert) => {
                assert.same(result, [], 'return empty resultset after delete referral');
            });
    },
    updateReferralStatus: function(stepname, contextStep) {
        return commonFunc.createStep('customer.referral.statusUpdate', stepname, context => customerParams.referralStatusUpdateParams(context, contextStep),
            (result, assert) => {
                assert.same(result, [], 'return empty resultset after delete referral');
            });
    }
