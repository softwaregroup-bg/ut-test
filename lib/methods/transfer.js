...
const transferParams = require('./../requestParams/transfer');
const transferJoiValidation = require('./../joiValidations/transfer');
...
module.exports = {
    ...
    transferFlowExecutePush: function(stepname, contextStep) {
        return commonFunc.createStep('transferFlow.push.execute', stepname, context => transferParams.transferFlowExecutePushParams(),
            (result, assert) => {
                assert.equals(transferJoiValidation.validateTransferFlowExecutePush(result).error, null, 'return all details for push execute');
            });
    }
};
