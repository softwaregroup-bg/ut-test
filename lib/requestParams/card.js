...
const SERVICECODE1 = [1, 2, 5, 6, 7, 9];
const SERVICECODE2 = [0, 2, 4];
const SERVICECODE3 = [0, 1, 2, 3, 4, 5, 6, 7];
const DECIMALISATION = '1234567890123456';
    ...
    addProductParams: function(context, productContext, productName, isActive) {
        return {
            product: {
                ...
                cardTypeId: productContext(context).cardTypeId,
                binId: productContext(context).binId,
                cardNumberConstructionId: productContext(context).cardNumberConstructionId,
                termMonth: productContext(context).termMonth || cardConstants.TERMMONTH,
                ...
                ownershipTypeId: productContext(context).ownershipTypeId,
                cipher: productContext(context).cipher,
                issuerId: productContext(context).issuerId,
                pinRetriesLimit: productContext(context).pinRetriesLimit || 5,
                accountLinkLimit: productContext(context).accountLinkLimit || 2,
                serviceCode1: SERVICECODE1[Math.floor(Math.random() * (SERVICECODE1.length))],
                serviceCode2: SERVICECODE2[Math.floor(Math.random() * (SERVICECODE2.length))],
                serviceCode3: SERVICECODE3[Math.floor(Math.random() * (SERVICECODE3.length))],
